...
//   const [certificateReviews, setCertificateReviews] = useState<IReview[]>([]);
...
//     try {
//       const response = await getAllCertificateReviews();
//       setCertificateReviews(response);
//     } catch (error) {
//       console.log(error);
//     }
//   };
...
//     try {
//       const response = await createCertificateReview(formData);
//       getReviews();
//       setCertificateReviews((prevState) => [...prevState, response]);
//       console.log(certificateReviews);
...
//                   >
//                     Відгуки ({certificateReviews.length})
//                   </span>
//                 </div>
//                 {activeTab === "about" && <CatalogCertificateAbout />}
//                 {activeTab === "reviews" && (
//                   <Reviews
//                     reviews={certificateReviews}
...
import { getAllCertificates } from "../../../services/gift-certificate/gift-certificate";
...
const CatalogCertificate: React.FC = () => {
  ...
  const getAll = async () => {
    try {
      ...
      setCertificateId(response[0].id);
      return response;
    } catch (error) {
      console.log(error);
    }
  ...
  const getReviews = async (certificateId: number) => {
    try {
      const response = await getAllCertificateReviews(certificateId);
      setCertificateReviews(response);
    } catch (error) {
      console.log(error);
    }
  ...
  const createReview = async (formData: FormData) => {
    if (certificateId !== null) {
      try {
        ...
        await getReviews(certificateId);
      } catch (error) {
        console.log(error);
      }
    } else {
      console.log("Certificate ID is not available");
    }
  };
  ...
  return (
    <>
          ...
          <div className={styles.catalog__certificate_wrapper}>
            <div className={styles.catalog__wrapper_route}>
              ...
              <p
                className={`${styles.catalog__router_name} ${styles.catalog__router_active}`}
              >
                {t("certificate.certificateRoute")}
              </p>
            </div>
            <div className={styles.catalog__certificate_main}>
              <CatalogCertificateGift
              ...
              <div className={styles.catalog__main_info}>
                <div className={styles.catalog__info_tabs}>
                  ...
                  <span
                    ...
                    onClick={() => setActiveTab("reviews")}
                  >
                    {t("products.productsTab3")} ({certificateReviews.length})
                  </span>
                </div>
                {activeTab === "about" && (
                  <CatalogCertificateAbout certificates={certificates} />
                )}
                {activeTab === "reviews" && (
                  <Reviews
                    reviews={certificateReviews}
      ...
      {isOpenReviewPopup && (
        <ReviewPopup
          createReviews={createReview}
          getReviews={() => getReviews(certificateId!)}
          isOpen={isOpenReviewPopup}
          onClose={toggleReviewPopup}
        />
      )}
